  ...
  return <div className="h-full bg-gray-100 relative">
      <div ref={scrollContainerRef} className="h-full overflow-y-auto p-6">
        <div className="max-w-4xl mx-auto space-y-2.5">
          ...
          return <div key={block.id} id={`block-${block.id}`} className={`relative transition-all duration-300 cursor-pointer ${!hasStarted ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`} onClick={() => onBlockSelect(block.id)} style={{
            transitionDelay: !hasStarted ? '0ms' : `${Math.max(0, block.startTime - currentTime)}ms`
          }}>
                  <div className="mx-[115px]">
                    {block.type === 'paragraph' ? <ParagraphBlock content={block.content as string} animationConfig={block.animationConfig} currentTime={adjustedCurrentTime} isActive={isActive} hasStarted={hasStarted} /> : <BulletListBlock content={block.content as {
                title: string;
                items: Array<{
                  bold: string;
                  desc: string;
                }>;
              }} animationConfig={block.animationConfig} currentTime={adjustedCurrentTime} isActive={isActive} hasStarted={hasStarted} />}
                  </div>
